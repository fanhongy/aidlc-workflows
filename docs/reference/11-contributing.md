...
1. **Create `core/scopes/aidlc-hotfix.md`** — the scope's identity. Frontmatter:
   ...
   - `freeform_default` (optional): set `true` to nominate this scope when the preferred core default (`classic`/`poc`) is not enabled. At most one enabled scope may claim it; graph compilation rejects ambiguous selected plugin sets. Unknown explicit `AWS_AIDLC_DEFAULT_SCOPE` values still fail validation.
