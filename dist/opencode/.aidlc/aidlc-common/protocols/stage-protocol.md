...
2. After the learnings answer is logged: `bun .aidlc/tools/aidlc.ts engine orchestrate report --stage <slug> --result awaiting-approval` — the engine marks `[-]` → `[?]` and emits `STAGE_AWAITING_APPROVAL`. `/aidlc --status` now truthfully shows the held gate.
...
compiled scope grid. Use `bun .aidlc/tools/aidlc.ts engine gen scope-table`
